...
export default new ApplicationCommand("backupEntry", async ({ application }) => {
  ...
  vscode.window.withProgress(
    ...
    async (progress) => {
      const { downloadURL } = (await cacheManager.pauseUntil(() => application.backup()))!;

      const backupBuffer = await axios
        .get(`https://registry.squarecloud.app/v1/backup/download/${downloadURL.split("/").pop()}`, {
          responseType: "arraybuffer",
        })
        .then((r: any) => r.data);

      writeFileSync(join(fsPath, `backup-${application.id}.zip`), Buffer.from(backupBuffer));
