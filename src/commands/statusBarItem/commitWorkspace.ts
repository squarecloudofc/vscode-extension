...
export default new Command("commitWorkspace", async () => {
  ...
  const application = cacheManager.applications.find((app) => app.id === applicationId);
  ...
  vscode.window.withProgress(
    ...
    async (progress) => {
      ...
      await cacheManager.pauseUntil(() => application.commit(zipFile.toBuffer(), "app.zip", true));
