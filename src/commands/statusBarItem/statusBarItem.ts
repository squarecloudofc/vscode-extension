...
export default new Command("statusBarItem", async () => {
  const workspaceApp = <string>configManager.defaultConfig.get("workspaceAppId");

  const application = cacheManager.applications.find((app) => app.id === workspaceApp);
  ...
  const { command } = Object.values(options).find(({ label }) => label === selected.label)!;
